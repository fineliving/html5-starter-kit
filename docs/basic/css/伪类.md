...
2.  border-box代表IE盒模型：盒子定义宽高 = 内容宽高（content） + 内边距 （padding）+ 边框（border）
...
理论上说，content-box与border-box区别是两者的盒子的**宽度**是否包含**边框**和**内边距**。

在控制台可以一目了然的看出两个盒子的区别：

1. **盒子定义宽高 = 内容宽高（content），100**，第一个div的盒子模型如下：

   ![content-box](/basic/css/content-box.png)

2. **盒子定义宽高 = 内容宽高（content） + 内边距 （padding）+ 边框（border），100=78+1\*2+10\*2**，第二个div的盒子模型如下：
